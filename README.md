...
默认配置位于 `contra/setting.py`。项目支持本地和服务器两种运行模式，默认使用 `auto` 自动判断：
...
运行模式的判断顺序如下：命令行 `--mode`、环境变量 `KLJP_MODE`、项目根目录下的
`.kljp-server` 标记文件、常见集群调度变量（如 `SLURM_JOB_ID`），以及服务器常见挂载路径
（`/data`、`/scratch`、`/lustre`、`/work`）或服务器模型挂载目录。没有服务器信号时使用本地模式。

```bash
# 自动判断（默认）
python contra/train_bl.py --mode auto

# 显式选择
python contra/train_bl.py --mode local
python contra/train_bl.py --mode server
```

也可以通过环境变量覆盖模式和路径，适合服务器提交任务或多机部署：

```bash
export KLJP_MODE=server
export KLJP_DATA_ROOT=/home/user/KLJP-DATA
export KLJP_LOG_ROOT=/home/user/KLJP-logs
export KLJP_ELECTRA_PATH=/mntF/XJJ/KLJP-master1/chinese-electra-180g-small-discriminator
python contra/train_bl.py
```

本地模式默认将训练输出写入项目内的 `logs/`；服务器模式默认写入
Ubuntu 目录 `/home/user/KLJP-logs/`，数据集和缓存默认读取/写入
`/home/user/KLJP-DATA/`。对应的 Windows WSL 访问路径为：

```text
\\wsl.localhost\Ubuntu-20.04\home\user\KLJP-logs
\\wsl.localhost\Ubuntu-20.04\home\user\KLJP-DATA
```

服务器首次运行前可执行：

```bash
mkdir -p /home/user/KLJP-logs /home/user/KLJP-DATA
cp -a KLJP-DATA/. /home/user/KLJP-DATA/
```

服务器模式按 RTX 4090 / 128 GB 内存配置为 batch size 16、4 个 DataLoader worker、
CUDA pinned memory；这些值仍可通过 `KLJP_BATCH_SIZE`、`KLJP_NUM_WORKERS` 和
`KLJP_DEVICE` 覆盖。每次运行生成的 `config.json` 会记录最终模式、判断原因和实际路径。

Electra 路径按模式区分：

```text
本地:   /mnt/f/XJJ/KLJP-master1/chinese-electra-180g-small-discriminator
服务器: /mntF/XJJ/KLJP-master1/chinese-electra-180g-small-discriminator
```

服务器的 `/mntF` 仅用于读取模型。训练过程不会向 `/mnt` 写入；缓存、日志、模型 checkpoint、
预测结果和 TensorBoard 文件都写入 `/home/user` 下的上述目录。

本地目录对应 Windows 路径：

```text
F:\XJJ\KLJP-master1\chinese-electra-180g-small-discriminator
```

服务器模式强制 Hugging Face/Transformers 离线，只允许读取上述本地模型，目录不存在时立即报错；
本地模式在目录不存在或读取失败时仍可回退到
`hfl/chinese-electra-180g-small-discriminator`。如需覆盖默认行为，可设置
`KLJP_ELECTRA_PATH`；`KLJP_ALLOW_MODEL_DOWNLOAD` 仅对本地模式生效。
...
每次训练会在当前模式的日志根目录下生成独立目录：

```text
<log_root>/<model_name>/<run_id>/
├── config.json
├── train.log
├── test.log
├── metrics.jsonl
...
本地模式的默认日志根目录为项目内的 `logs/`。查看 TensorBoard 时，将
`<log_root>` 替换为实际日志根目录：
